  ...
  if (controlMode == 1) {
    ...
    if (mode == 6) {
      ...
      if (startTriggerTime > 0) {
        ...
        checkTime = micros();
        if (checkTime - startTimeout > 30000000) {
          Serial.println("ERROR: Master timeout after 30 seconds, resetting to calibrate");
          BTSerial.println("Timeout30");
          mode = 5;
          data.mode = 5;
          controlMode = 0;
          radio.stopListening();
